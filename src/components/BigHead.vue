...
export default {
  ...
  computed: {
    properties() {
      ...
      const extras = this.$bigHeadsExtras?.() || {};
      const props = this.props;

      for (let i = 0; i < props.length; i++) {
        const extras_map = extras[props[i]];

        for (const prop in extras_map) {
    ...
    values() {
      const shape = this.validateProperty(this.shape, 'shape');
      let mask = null;
  ...
  methods: {
    ...
    validateProperty(value, prop) {
      ...
      if (this.properties[prop].is_color === true) {
        return this.validateColor(value, prop);
      }

      if (!Object.keys(this.properties[prop].map)
        .includes(value)) {
